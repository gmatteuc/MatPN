function [ aspect_ratio, eccentricity, orientation, contrast_factor, fig_handl  ] = get_shape_params( input_image, bool_plot )
% [ aspect_ratio, eccentricity, orientation, contrast_factor, fig_handl ] = get_shape_params( input_image, bool_plot )
...
if isempty(Istats)
    ...
    % decide witch connected region to use
    ...
    cm=Istats(mesize_idx).Centroid;
    or=Istats(mesize_idx).Orientation; % returns a scalar that specifies the angle between the x-axis and the major axis of the ellipse that has the same second-moments as the region. Value is in degrees, ranging from -90 to 90 degrees.
    ma=Istats(mesize_idx).MajorAxisLength;
    mi=Istats(mesize_idx).MinorAxisLength;
    ar=mi/ma;
    ec=Istats(mesize_idx).Eccentricity;
    su=Istats(mesize_idx).Area;
